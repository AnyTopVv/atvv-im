...
//        TODO：路径有效性校验
        ...
        if (tokens == null || tokens.size() == 0) {
            DataBuffer buffer = ResponseUtil.getResponseBuffer(response, "请求头缺少token!", null);
        ...
        if (claims.getExpiration().before(new Date())) {
            ...
            DataBuffer buffer = ResponseUtil.getResponseBuffer(response, "token已过期!", null);
            return response.writeWith(Mono.just(buffer));
        }
        ...
        if (Objects.isNull(user)) {
            ...
            DataBuffer buffer = ResponseUtil.getResponseBuffer(response, "用户未登录!", null);
            return response.writeWith(Mono.just(buffer));
        }
